...
class HelpCategoryController extends AbstractController
{
    ...
    )
    {
        ...
        if ($request->query->has('parentId'))
            $qb = $em->getRepository(HelpCategory::class)->getAdminWithParent($search, $request->query->get('parentId'));
        else
            $qb = $em->getRepository(HelpCategory::class)->getAdminWithParent($search);

        $categories = $paginator->paginate($qb, $request->query->getInt('page', 1), 25);

        $parent = ($request->query->has('parentId')) ? $em->getRepository(HelpCategory::class)->find($request->query->get('parentId')): null;

        return $this->render('admin/helpCategory/index.html.twig', [
    ...
    )
    {
        ...
        if ($request->query->has('parentId'))
            $parent = $em->getRepository(HelpCategory::class)->find($request->query->get('parentId'));

        $category = new HelpCategory();
        $category->setParent($parent);

        $form = $this->createForm(HelpCategoryType::class, $category);
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $redirect = ($request->query->has('parentId')) ? $this->redirectToRoute('app_admin_help_category_index', ['parentId' => $request->query->get('parentId')]) : $this->redirectToRoute('app_admin_help_category_index');

            return $redirect;
        }

        return $this->render('admin/helpCategory/create.html.twig', [
    ...
    {
        $category = $em->getRepository(HelpCategory::class)->find($id);
        $form = $this->createForm(HelpCategoryType::class, $category);
        ...
        return $this->render('admin/helpCategory/edit.html.twig', [
            'form' => $form->createView(),
            'category' => $category,
     ...
     */
    private function deleteForm(HelpCategory $category)
    {
        return $this->createFormBuilder()
            ->setAction($this->generateUrl('app_admin_help_category_delete', ['id' => $category->getId()]))
            ->setMethod('DELETE')
            ->getForm();
    }
    ...
    {
        return $this->createFormBuilder()
            ->setAction($this->generateUrl('app_admin_help_category_bulk_delete'))
            ->setMethod('DELETE')
            ->getForm();
    }
