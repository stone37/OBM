...
use App\Form\AdminDiscountType;
...
class DiscountController extends AbstractController
{
    ...
    {
        $qb = $em->getRepository(Discount::class)->getAdmin();

        $discounts = $paginator->paginate($qb, $request->query->getInt('page', 1), 25);
        ...
        EventDispatcherInterface $dispatcher)
    {
        ...
        $form = $this->createForm(AdminDiscountType::class, $discount);
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $this->addFlash('info', 'Un code de réduction a été crée');
        ...
        EventDispatcherInterface $dispatcher, $id): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $this->addFlash('info', 'Un code de réduction a été mise à jour');
        ...
        $id)
    {
        ...
        if ($request->getMethod() == 'DELETE') {
            ...
            if ($form->isSubmitted() && $form->isValid()) {
                ...
                $this->addFlash('info', 'Le code de reduction a été supprimé');
            } else {
    ...
    )
    {
        ...
        if ($request->getMethod() == 'DELETE') {
            ...
            if ($form->isSubmitted() && $form->isValid()) {
                ...
                foreach ($ids as $id) {
                    $discount = $em->getRepository(Discount::class)->find($id);
                    $dispatcher->dispatch(new AdminCRUDEvent($discount), AdminCRUDEvent::PRE_DELETE);
                ...
                $this->addFlash('info', 'Les codes de réduction ont été supprimé');
            } else {
