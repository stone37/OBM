$(document).ready(function() {
    ...
    switch (category) {
        case "voiture-doccasion":
            ...
            if (brand) {
                ...
                $.each($autoModel[brand], function(i, obj) {
                    let $content;
    ...
    // Bouton filter and alert scroll
    $(window).scroll(function() {
        let scroll = $(window).scrollTop();
        let currScrollTop = $(this).scrollTop();
        let $parent = $('#alert-filter-scroll');

        if (scroll >= 300) {
            $parent.removeClass('d-none');
            $parent.find('.mobile').addClass('animated fadeInRight');
        } else {
            $parent.addClass('d-none');
            $parent.find('.mobile').removeClass('animated fadeInRight');
        }
